...
function systemPrompt(assetNames: Record<string, string[]>): string {
  return [
    ...
    '  npc.loot        loot dropped when defeated: a name like "Coins", "Gold Bag", "Gem Blue".',
    '  npc.wantsItem   fetch quest: loot kind this NPC asks for (e.g. "Chest Gold").',
    '  npc.thanksLine  said when the player delivers wantsItem.',
    '  npc.reward      loot kind granted on delivery.',
    '',
    'RECIPES — how the game reads these fields:',
    '  Damage: anything with an animation rig can be punched or hit by a thrown',
    '    prop. It does NOT need an npc block for that.',
    '  Fights: npc.faction decides who starts them. hostile chases and attacks;',
    '    neutral only fights back once struck; friendly never attacks.',
    '  Talking: needs npc.lines. Add canFollow for a companion offer, or',
    '    guideTo + arriveLine for an escort.',
    '  Throwables: pickable:true (E picks up, F throws).',
    '  Loot: npc.loot drops on defeat. Treasure (Coins, Gems, Gold Bag, Gold ore,',
    '    Chest Gold) auto-collects into the player inventory when walked over.',
    '  Fetch quest: wantsItem + thanksLine + reward on a friendly NPC.',
    '  Travel: exitTo on any entry ("pirate-cove", "fishing-village", "jungle-outpost",',
    '    "monster-arena", "blank-sands") makes it a portal — E sails there.',
    '  Patrol: path + speed. A brain state (chase/follow/guide) overrides it.',
    '  Scenery that should not block the player: solid:false — always use this',
    '    for grass, flowers, ferns and pebbles.',
    '',
